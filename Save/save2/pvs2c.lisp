...
(defmethod pvs2C* ((expr if-expr) bindings livevars)
  (let* ((type (pvs-type-2-C-type expr))
	 (C-expr (pvs2C-if expr bindings livevars type nil)))
    (cons type
	  (if (pointer? type)
	      (cdr C-expr)
	    (let ((if-name (gentemp "if")))
	      (add-instructions (apply-argument (cdr C-expr) if-name))
	      (list if-name))))))

(defmethod pvs2C ((expr if-expr) bindings livevars exp-type)
  (let* ((type (pvs-type-2-C-type expr))
	 (C-expr (pvs2C-if expr bindings livevars type t))
	 (if-name (gentemp "if")))
    (add-instructions (apply-argument (cdr C-expr) if-name))
  ...
  (let ((name (getConstantName expr bindings)))
	...
	(if (pointer? type)
	  ...
	  (let* ((l (length *C-destructions*))
		 (e (pvs2C* expr bindings livevars nil))) ;; Return an expression
	    (if (eq l (length *C-destructions*)) ; If it's a very simple expression
		(convert exp-type (C-type e) e )
	      (let* ((type (C-type e))
		     (n (gen-C-var type "aux")))
		(add-instructions (C-alloc n))
		(add-instruction (format nil "~a ~a = ~a;" type n e))
		(add-instructions *C-destructions*)
		(reset-destructions)
		(add-destructions (C-free n))
...
;; converter need to be completed here
...
(defmethod pvs2C* ((expr number-expr) bindings livevars result)
  ...
  (let* ((type (slot-value result 'type))
	 (op (cond ((eq (type-of type) 'C-mpz) "mpz_set_str")
		   ((eq (type-of type) 'C-mpq) "mpq_set_str")
		   (t "setUnknown"))))
    (format nil "~a(~a, \"~a\");" op result (number expr))))
...
(defmethod pvs2C* ((expr tuple-expr) bindings livevars result)
  (let ((args (pvs2C (exprs expr) bindings livevars)))
    (if (pointer? result)
	(mk-C-funcall "createTuple" (append (list result) args))
      "[Tuples are pointers]")))
...
(defmethod pvs2C*  ((expr field-application) bindings livevars result)
  (let* ((clarg (pvs2C (argument expr) bindings livevars ))
	 (id (id expr)))
    (set-C-var "set" result (format nil "~a.~a" clarg id))))
    ...
    (if (constant? operator)
	  ...
	  (let* ((bind-decls (bindings operator))
	    ...
	    (add-instructions (mapcar
			       #'(lambda (x) (format nil "~a ~a = ~a;"
						     ...
						     (car x)
						     ;; (pvs2C* (car x) newbind nil "unimplemented")
						     (cdr x)))
			       (pairlis (bindings operator)  C-arg)))
	    (pvs2C* (expression operator) newbind nil result))
	 (let ((C-arg (pvs2C argument bindings
		...
		(format nil "pvsSelect(~a, ~a);" C-op C-arg)
	        (set-C-pointer C-op result C-arg)))))))
...
(defun pvs2C-defn-application (expr bindings livevars result)
  (with-slots (operator argument) expr
    ...
    (let* ((actuals (expr-actuals (module-instance operator)))
	  ...
	  (let* ((defns (def-axiom op-decl))
	    ...
	    (set-C-funres (if check (C_id operator)
			            (C_nondestructive_id operator))
			  result
			  C-args))
	(set-C-funres (C_id operator) result C-args))))) ;;this should be
;;C_nondestructive_id?
...
(defun pvs2C-lambda (bind-decls expr bindings result) ;;removed livevars
  (let ((previous-instr *C-instructions*))
	...
	(add-instructions (mapcar #'(lambda (x) (format nil "args[~d] = ~a;"
						      (cdr x) (id (car x))))
				assign))
	(format nil "makeClosure(~a, ~a, args)" result name)))))
