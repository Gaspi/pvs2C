...
(defmethod pvs2C* ((expr number-expr) bindings livevars)
  ...
  (format nil "pvsInt(\"~a\")" (number expr)))
...
;;  Shankar or Sam should check this code !!!
;;(defun pvs2C-let (vars exprs bindings livevars)
;;  (if vars
;;     (format nil "~a ~a = ~a;~%~a"
;;	 (pvs2C-type (type (car vars)))
;;	 (pvs2C* (car vars)  bindings livevars )
;;	 (pvs2C* (car exprs) bindings livevars )
;;	 (pvs2C-let (cdr vars) (cdr exprs) bindings livevars ))
;;     ""))

;;(defmethod pvs2C* ((expr let-expr) bindings livevars)
;;  (with-slots (operator argument) expr
;;    (let* ((newlivevars (append (updateable-free-formal-vars operator) livevars))
;;	  (C-decl (pvs2C-let (bindings operator) argument bindings newlivevars))
;;	  (C-expr (pvs2C* (expression operator) bindings
;;			      (append (updateable-vars
;;					argument)
;;					livevars))))
;;      (format nil "~a~a" C-decl C-expr))))
	  ...
	  (let* ((bind-decls (bindings operator))
	    ...
	    (add-instructions (mapcar
			       #'(lambda (x) (format nil "~a ~a = ~a;"
						     ...
						     (pvs2C* (car x) newbind nil)
						     (cdr x)))
			       (pairlis (bindings operator)  C-arg)))
	    (pvs2C* (expression operator) newbind nil))
	   (let ((C-arg (pvs2C* argument bindings
			   (append (updateable-free-formal-vars operator) livevars)))
...
(defun pvs2C-primitive-app (expr bindings livevars)
  (let* ((op (operator expr))
    ...
    (if (pvs2C-primitive-infix? op)
	(if (eq nop '==)
	    (format nil (pvs2C-eq (type (car (arguments expr)))) (car args) (cadr args) )
	  (format nil (format nil "(~a~a~a ~a ~a)" "~{" "~a" "~^" nop "~}")  args ))
        (if (and (= (length args) 1) (eq nop 'pvsSub))
	    (format nil "pvsNeg(~a)" (car args))
	  (format nil "~a(~{~a~^, ~})" nop args)))))

(defmethod pvs2C-eq (type)
  (cond ((subtype-of? type *integer*) "intEq(~a, ~a)"  )
	((subtype-of? type *real*   ) "ratEq(~a, ~a)")
	((funtype? type  ) (if (C-updateable? type) "arrayEq(~a, ~a)"
			     "closureEq(~a, ~a)"))
	((tupletype? type) "tupleEq(~a, ~a)")
	(t "~a == ~a")))
...
(defun pvs2C-resolution-nondestructive (op-decl formals body range-type)
  (let* ((*destructive?* nil)
	 ...
	 (cl-body (pvs2C* body id-map nil))
	 (cl-type-out (format nil "~a" (pvs2C-type range-type)))
	 (cl-type-arg (format nil "~{~a~^, ~}"
		  (loop for var in formals
			collect (format nil "~a ~a"
			      (pvs2C-type (type var)) (cdr (assoc var id-map)) ))))
	 (cl-defn (format nil "~a" cl-body))
	 (hash-entry (gethash op-decl *C-nondestructive-hash*)))
    (format t "~%Defining (nondestructively) ~a with ~%type ~a -> ~a ~%as ~a" (id op-decl) cl-type-arg cl-type-out cl-defn)
    (setf (C-info-type-out hash-entry)
	  cl-type-out
	  (C-info-type-arg hash-entry)
	  cl-type-arg
	  (C-info-definition hash-entry)
	  (format nil "~{  ~a~%~}  return ~a;~%" *C-instructions*  cl-defn)
	  )
    (reset-instructions)
      ))
...
(defun pvs2C-resolution-destructive (op-decl formals body range-type)
  (let* ((*destructive?* t)
	 ...
	 (old-output-vars (C-info-analysis hash-entry)))
        (format t "~%Defining (destructively) ~a with ~%type ~a -> ~a ~%as ~a" (id op-decl) cl-type-arg cl-type-out cl-defn)
    (setf (C-info-type-out hash-entry)
	  cl-type-out
	  (C-info-type-arg hash-entry)
	  cl-type-arg
...
(defun pvs2C-lambda (bind-decls expr bindings) ;;removed livevars
  (let ((previous-instr *C-instructions*))
	...
	(add-instructions (mapcar #'(lambda (x) (format nil "args[~d] = ~a;"
						      (cdr x) (id (car x))))
				assign)) 
	(format nil "makeClosure(~a, args)" name)))))
     ...
     (let* ((if-result (gentemp "if"))
	    ...
	    (newlivevars (append (updateable-vars then-part)
				 (append (updateable-vars else-part)
					 livevars)))
	    (C-condition (pvs2C* condition bindings newlivevars))
	    (previous-instr *C-instructions*))
       (reset-instructions)
       (let ((C-then-part (pvs2C* then-part bindings livevars))
	     (C-then-instructions *C-instructions*))
	 (reset-instructions)
	 (let ((C-else-part (pvs2C* else-part bindings livevars))
	       (C-else-instructions *C-instructions*))
	   (reset-instructions)
	   (add-instructions previous-instr)
	   (add-instruction (format nil "~a ~a;" (pvs2C-type (type expr)) if-result))
	   (add-instruction (format nil "if(~a) {" C-condition))
	   (add-instructions (mapcar
			      #'(lambda (x) (format nil "  ~a" x))
			      C-then-instructions))
	   (add-instruction (format nil "  ~a = ~a;" if-result C-then-part))
	   (add-instruction "} else {")
	   (add-instructions (mapcar
			      #'(lambda (x) (format nil "  ~a" x))
			      C-else-instructions))
	   (add-instruction (format nil "  ~a = ~a;" if-result C-else-part))
	   (add-instruction "}")
  ...
  (with-slots (type expression assignments) expr
    ...
    (pvs2C-update* (type expression)
				   ...
				   (append (updateable-vars expression)
					   livevars)
				   (list (list (pvs2C-type type) exprvar C-expr)))))))
...
(defun pvs2C-update-nd-type (type expr newexprvar args assign-expr
  ...
  (if (consp args)
      (pvs2C-update-nd-type* type expr newexprvar (car args) (cdr args) assign-expr
			      bindings livevars accum)
      (cons (list (pvs2C-type type) newexprvar assign-expr) accum)))
  ...
  (let* ((arg1var (gentemp "L"))
	 (C-arg1 (pvs2C*  (car arg1) bindings
				 (append (updateable-vars restargs)
					 livevars))))
    (push (list (pvs2C-type (type (car arg1))) arg1var C-arg1) *lhs-args*)
    (if (consp restargs)
	(let* (
	       (exprvar (gentemp "E"))
		...
		(pvs2C-update-nd-type 
		 ...
		 (cons (list (pvs2C-type type) exprvar exprval) accum))))
	  (cons (list (pvs2C-type type) newexprvar (pvsC_update expr arg1var newexprvar2))
		newaccum)
	  )
	(cons (list (pvs2C-type type) newexprvar (pvsC_update expr arg1var assign-expr))
	      accum))))
	...
	(if (consp restargs)
	    (let* (
		   ...
		   (newaccum (pvs2C-update-nd-type field-type exprvar newexprvar2
						       ...
						       livevars
						       (cons (list (pvs2C-type type) exprvar new-expr) accum))))
	      (cons (list (pvs2C-type type) newexprvar (format nil "{~a & ~a = ~a}" expr id newexprvar2)) newaccum))
	    (cons (listn (pvs2C-type type) ewexprvar (format nil "{~a & ~a = ~a}" expr id assign-expr))
		  accum))))
	...
	(dolist (decl (theory theory))
	  (let ((ndes-info (gethash decl *C-nondestructive-hash*))
		...
		(format output  "~2%~a ~a(~a) {"
			(C-info-type-out ndes-info) id (C-info-type-arg ndes-info))
		(format outputH "~2%~a ~a(~a);"
			(C-info-type-out ndes-info) id (C-info-type-arg ndes-info))
		;; Then the defn
		(format output "~%~a}"  (C-info-definition des-info))))
		;;(format output "~%~{  ~#[~;return ~:;~]~a;~%~}}"
	;;		(line-split (C-info-definition ndes-info)))
	    (when des-info
	      (let ((id (C-info-id des-info)))
		;; First the signature
		(format output  "~2%~a ~a(~a) {"
			(C-info-type-out des-info) id (C-info-type-arg des-info))
		(format outputH "~2%~a ~a(~a);"
			(C-info-type-out des-info) id (C-info-type-arg des-info))
		;; Then the defn
		(format output "~%~a}"  (C-info-definition des-info)))))))))))
